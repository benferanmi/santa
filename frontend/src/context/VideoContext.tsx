...
export const VideoProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const createVideoOrder = useCallback(
    async (
      videoData: VideoRequest
    ): Promise<VideoGenerationResponse | null> => {
      setLoading(true);
  ...
  const getVideoOrder = useCallback(
    async (orderId: string): Promise<VideoOrder | null> => {
      setLoading(true);
      setError(null);
      try {
  ...
  const cancelVideoOrder = useCallback(
  ...
  const shareVideo = useCallback(
    async (
      ...
      setError(null);
      try {
        const response = await VideoApiService.shareVideo(
          orderId,
          expirationDays
        );
        if (response.success && response.data) {
